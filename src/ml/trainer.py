...
from src.ml.dataset import TextClassificationDataset
...
class BertTrainer:
    ...
    def create_data_loaders(
        ...
        logger.info(
            f"数据加载器创建完成 - 训练批次: {len(train_loader)}, 验证批次: {len(val_loader)}"
        )
    ...
    def train(
        ...
        """训练模型"""
        logger.info(
            f"开始训练 - 轮数: {epochs}, 批次大小: {batch_size}, 学习率: {learning_rate}"
        )
