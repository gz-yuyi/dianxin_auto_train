...
class Hyperparameters(BaseModel):
    ...
    max_sequence_length: int = Field(default=512, ge=1, le=512, description="最大序列长度")
    random_seed: int = Field(default=42, description="随机种子")
    train_val_split: float = Field(default=0.2, ge=0, le=1, description="训练验证集划分比例")
...
class CreateTrainingTaskRequest(BaseModel):
    """创建训练任务请求"""
    model_name_cn: str = Field(..., min_length=1, max_length=100, description="模型中文名称")
    model_name_en: str = Field(..., pattern=r"^[a-zA-Z0-9_]+$", description="模型英文名称")
    training_data_file: str = Field(..., description="训练数据文件名")
    base_model: BaseModelEnum = Field(default=BaseModelEnum.BERT_BASE_CHINESE, description="基础模型")
    hyperparameters: Hyperparameters = Field(default_factory=Hyperparameters, description="超参数")
