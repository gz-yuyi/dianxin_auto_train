...
from src.api.routes.training_simple import router as training_router
...
def create_simple_app() -> FastAPI:
    ...
    app = FastAPI(
        title="BERT训练API（简化版）",
        description="基于BERT模型的文本分类训练服务（演示版本，无ML功能）",
        version="0.1.0",
    ...
    async def root():
        return {
            "message": "BERT训练API服务（简化版）",
            "version": "0.1.0",
            "docs": f"{API_PREFIX}/docs",
            "note": "这是演示版本，ML功能需要安装额外依赖"
        }
    
    logger.info("简化版FastAPI应用创建完成")
    return app
