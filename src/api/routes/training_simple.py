...
from src.api.models.requests import CreateTrainingTaskRequest
...
from src.services.training_simple import training_service_simple as training_service
...
def task_to_response(task) -> TrainingTaskResponse:
    ...
    return TrainingTaskResponse(
        ...
        if task.status in [TaskStatus.TRAINING, TaskStatus.COMPLETED]
        else None,
...
async def create_training_task(request: CreateTrainingTaskRequest):
    ...
    try:
        ...
        # 模拟训练开始（实际使用时需要安装ML依赖）
        logger.info(
            f"任务创建成功，但需要安装ML依赖才能开始实际训练 - 任务ID: {task.task_id}"
        )
        ...
        return CreateTrainingTaskResponse(
            ...
            status=task.status,
            created_at=task.created_at,
            message="训练任务已创建（需要安装ML依赖才能开始实际训练）",
        )
...
async def get_task_list(
    ...
    return TaskListResponse(
        total=total, page=page, page_size=page_size, tasks=task_responses
    )
...
async def stop_training_task(task_id: str):
    ...
    if not success:
        ...
        raise HTTPException(
            status_code=400, detail="无法停止任务，任务可能不存在或状态不允许"
        )

    task = training_service.get_task(task_id)
    return SimpleTaskResponse(
        task_id=task_id, status=task.status, message="训练任务已停止（模拟）"
    )
...
async def delete_training_task(task_id: str):
    ...
    if not success:
        ...
        raise HTTPException(
            status_code=400, detail="无法删除任务，任务可能不存在或正在训练中"
        )

    return SimpleTaskResponse(
        task_id=task_id, status=TaskStatus.STOPPED, message="训练任务已删除"
    )
